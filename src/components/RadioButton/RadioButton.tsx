...
interface RadioButtonProps extends Pick<InsuranceOption, "title" | "description" | "price" | "recommended" > {
    selected: boolean;
    onChange: () => void;
  }
  ...
  onChange,
    ...
    return (
        <label className={`radio-button ${selected ? "selected" : ""} ${recommended ? "recommended" : ""}`}>
            ...
            {selected && description && <p className="radio-description">{description}</p>}
          </div>
        </label>
      );
    };
